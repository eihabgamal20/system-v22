        ...
        switch (optionhandletype) {
          case "Channel Welcome Messages": {
            ...
            async function second_layer() {
              ...
              let MenuEmbed = new MessageEmbed()
                .setColor(es.color)
                .setAuthor('Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png', 'https://discord.gg/t4nmUV3raw')
                .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
              //send the menu msg
              let menumsg = await message.reply({ embeds: [MenuEmbed], components: [new MessageActionRow().addComponents(Selection)] })
              //Create the collector
            ...
            async function dm_msg_autoimg(member) {
              ...
              try {
                ...
                if (client.settings.get(member.guild.id, "welcome.membercountdm")) {
                  await canvacord.Util.renderEmoji(ctx, `بعدك بقينا${member.guild.memberCount}عضو`, 750, canvas.height / 2 + 200);
                }
                //get the Guild Name
